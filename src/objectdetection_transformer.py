...
def msg_callback(message):
    ...
    global_msg.class_type = local_msg.class_type
    global_msg.index = local_msg.index
    global_msg.width = local_msg.width
    global_msg.height = local_msg.height
    global_msg.depth = local_msg.depth
    global_msg.image = local_msg.image

    objDetection_pub.publish(global_msg)
...
if __name__ == "__main__":
    ...
    rate = rospy.Rate(1.0)
    objDetection_pub = rospy.Publisher("global_objDetection",ObjectDetectionMsg, queue_size=5)  

    rospy.Subscriber("local_objDetection", ObjectDetectionMsg, msg_callback) 
    global_msg = ObjectDetectionMsg()  
